...
public class BookInfodto {
	...
	LocalDateTime datetime; // 도서 출판 날짜 [YYYY]-[MM]-[DD]T[HH]:[mm]:[ss].000+[tz]
	...
	public static BookInfodto parse(JSONObject input) {
		BookInfodto info = new BookInfodto();
		if(input.has("authors")) {
			JSONArray authorsArr = input.getJSONArray("authors");
			String []authors = new String[authorsArr.length()];
			for(int i=0; i<authorsArr.length(); i++) {//배열 닮는작업
